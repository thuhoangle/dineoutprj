...
export const CheckTableSlotDrawer: FC<CheckTableSlotDrawerProps> = ({
  isOpen,
  onClose,
  data,
}) => {
  const popoverRef = useRef<HTMLDivElement>(null);
  const selectedTableEvents = useEventStore(
    (state) => state.selectedTableEvents
  );

  // Group events by date
  const eventsByDate = selectedTableEvents.reduce(
    (acc, event) => {
  ...
  const totalSlots = selectedTableEvents.length;
        ...
        <DrawerHeader className="flex flex-col gap-2 pb-3">
          ...
          <div className="flex items-center justify-between">
            <div className="text-lg font-medium">
              Table #{data.tables.table_number}
            </div>
          </div>
            ...
            {data.tables.seat_type && (
              <div className="flex items-end gap-2 text-gray-500 text-sm">
                ...
                <div className="font-medium">
                  {upperFirst(data.tables.seat_type)}
                </div>
              </div>
            )}
            {data.more_info && (
              <div className="flex items-start gap-2 text-gray-500 text-sm">
                ...
                <div className="font-medium flex-1">{data.more_info}</div>
              </div>
            )}
          </div>
        </DrawerHeader>
          ...
          <div className="flex flex-col gap-4 py-2">
            {sortedDates.map((date) => (
              <div key={date} className="flex flex-col gap-2">
                <div className=" text-gray-700">
                  {totalSlots} available {totalSlots === 1 ? 'slot' : 'slots'}{' '}
                  across {datesCount} {datesCount === 1 ? 'day' : 'days'}
                </div>
            ...
            {Object.keys(eventsByDate).length === 0 && (
              <div className="text-gray-500">
                No available time slots for this table.
              </div>
            )}
          </div>
        </DrawerBody>
