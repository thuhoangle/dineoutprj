...
export const anyToNumString = (input: any) => {
  ...
  if (str.includes('e')) {
    ...
    if (exponent < 0) {
      ...
      str =
        integerPart + decimalPart + '0'.repeat(exponent - decimalPart.length);
    }
  }
  ...
  if (fixed || fixed === 0) {
    ...
    if (arr?.[1]?.length) {
      if (arr[1].length > fixed)
        calcValue = str.slice(0, str.length - arr[1].length + fixed);
      else calcValue = str + '0'.repeat(fixed - arr[1].length);
    } else if (fixed) calcValue = `${str}.${'0'.repeat(fixed)}`;
  }

  if (trimZeros && calcValue.includes('.'))
    calcValue = calcValue.replace(/0+$/g, '');
...
export const removeTrailingZeros = (s: string) =>
  s.includes('.') ? s.replace(/(\.\d*?[1-9])0+$|\.0*$/, '$1') : s;

export const trimZeros = (s: string) =>
  s.replace(/^0+(?=\d)|(\.\d*?[1-9])0+$|(\.0*|\.)(?=$)/g, '$1') || '0';
...
export const floatToFixed = (
  ...
  if (fixedNumber || fixedNumber === 0) {
    ...
    if (arr?.[1]?.length) {
      const fixed =
        arr[1].length > fixedNumber ? arr[1].length - fixedNumber : 0;
...
export const convertInputToFloatWithIncrement = (
...
export const convertNumberInput = (
  inputStr: string | number | undefined,
  increment?: string | number
) => {
  if (!inputStr) return '';
...
export const displayElipsisAddress = (
  address: string,
  start: number,
  end: number
) => {
...
export const formatUnit = (inputValue: number) => {
  ...
  const formatted =
    tier <= 0
      ? value.toString()
      : scaled.toFixed(scaled < 10 ? 2 : scaled < 100 ? 1 : 0); // Adjust precision
